...
/**
 * 类名称：BusinessDBConfigManager<br>
 * 类描述：<br>
 * 创建时间：2018年12月27日<br>
 *
 * @author maopanpan
 ...
 */
@Configuration
@MapperScan(basePackages = "com.myself.test.mapper.business", sqlSessionTemplateRef = "businessSqlSessionTemplate")
public class BusinessDBConfigManager {

    // 配置数据源
    @Bean(name = "businessDataSource")
    public DataSource testDataSource(BusinessDBConfig dbConfig) throws SQLException {
    ...
    public SqlSessionFactory testSqlSessionFactory(@Qualifier("businessDataSource") DataSource dataSource)
            throws Exception {
        SqlSessionFactoryBean bean = new SqlSessionFactoryBean();
        bean.setDataSource(dataSource);
        bean.setTypeAliasesPackage("com.myself.test.domain.business");//指定基包
        bean.setMapperLocations(new PathMatchingResourcePatternResolver().getResources("classpath*:/mappings/business/*.xml"));//指定xml文件位置
    ...
    private Properties build(BusinessDBConfig dbConfig) {
